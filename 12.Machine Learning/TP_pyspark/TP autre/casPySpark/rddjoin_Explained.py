...
#print(citiesRDD.collect())
deptsRDD=spark.sql("select d.code, d.slug from departments d where d.code=67").rdd.map(list)
#deptsRDD.collect()
#monumentsRDD = sc.textFile("dbfs://file......csv").map(lambda p:p.split("|")).map(lambda p : (p(11), concat())
...
#monumentsRDD=spark.sql("select m.INSEE,TRIM(m.REF),m.STAT,m.DPRO,m.DPT,m.REG from monument m where m.INSEE = 67529").rdd.map(list)
#monumentsRDD.collect()

# COMMAND ----------

# premiere jointure
monuments_citiesRDD=monumentsRDD.join(citiesRDD)
print(monuments_citiesRDD.collect())
def splitCities(a):
  ar = a[1][0].split("__")
...
mRDD=sc.textFile("/FileStore/tables/palissy_MH_valid_csv-26231.utf").map(lambda p:p.split("|"))
header = mRDD.first() #extract header
mRDD = mRDD.filter(lambda row : row != header).map(lambda l :( l[2],"__".join(l)) ).map(lambda data:(data[0],data[1].split("__")[0].strip()))
mRDD.join(deptsRDD).collect()
