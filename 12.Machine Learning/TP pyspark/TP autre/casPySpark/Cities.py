...
dfMonument = sqlContext.table("monuments").select(clear_udf('REF').alias('NEW_REF'), col('REG').alias('REG'),col('COM').alias('COM'))
dfMonument.show(1)
...
def concat(arr):
  return ''.join(arr)
concat_cols = udf(lambda arr: concat(arr), StringType())
dfMonumentAsmaa = dfMonument.select(concat_cols(array("REG",'NEW_REF','COM')).alias("asmaa"))
dfMonumentAsmaa.show(1)
...
dfMonumentAsmaa = dfMonument.select(concat_cols(array('REF','REG','COM')).alias("asmaa"))
dfMonumentAsmaa.show(1)
